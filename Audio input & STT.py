...
from scipy.signal import butter, lfilter  # ✅ Used in high_pass_filter
...
def process_and_transcribe(raw_audio, filename_base):
    ...
    log.info("🔍 Running Voice Activity Detection...")
    ...
    with open(os.path.join(OUTPUT_DIR, f"{filename_base}_transcription.txt"), "w", encoding="utf-8") as f:
        f.write(result["text"])
...
def main():
    ...
    process_and_transcribe(raw_audio, filename_base)
